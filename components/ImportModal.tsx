 ...
 */
const ImportModal: React.FC<ImportModalProps> = ({ isOpen, isImporting = false, onClose, onConfirm, layers, initialLayerId, type }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={isImporting ? undefined : onClose}>
            <div className="bg-gray-800 border border-gray-700 rounded-xl shadow-2xl w-full max-w-md text-gray-200" onClick={e => e.stopPropagation()}>
                <header className="flex items-center justify-between p-4 border-b border-gray-700">
                    ...
                    <button onClick={isImporting ? undefined : onClose} className="text-gray-400 hover:text-white transition-colors text-2xl">&times;</button>
                </header>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center">
                                <input 
                                    ...
                                    disabled={isImporting} onChange={() => setScaleMode('original')}
                            ...
                            <div className="flex items-center">
                                <input 
                                    ...
                                    disabled={isImporting} onChange={() => setScaleMode('fit')}
                                    className="w-4 h-4 text-cyan-600 bg-gray-700 border-gray-600 focus:ring-cyan-500 focus:ring-2"
                                />
                ...
                <footer className="flex justify-end p-4 bg-gray-900/50 border-t border-gray-700 space-x-3">
                    <button onClick={isImporting ? undefined : onClose} className="px-4 py-2 rounded-lg bg-gray-600 hover:bg-gray-500 text-white font-semibold transition-colors">Cancel</button>
                    <button onClick={handleConfirm} disabled={isImporting} className="px-6 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-semibold transition-colors">{isImporting ? 'Importing…' : 'Import'}</button>
                </footer>
            </div>
        </div>
    );
